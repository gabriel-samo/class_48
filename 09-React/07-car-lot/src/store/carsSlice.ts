...
export const carsSlice = createSlice({
    ...
    reducers: {
        addCar(state, action: PayloadAction<Car>) {
            const carsIndex = state.cars.findIndex((item) => item.mispar_rechev === action.payload.mispar_rechev);
        ...
        removeCar(state, action: PayloadAction<number>) {
            const carsIndex = state.cars.findIndex((item) => item.mispar_rechev === action.payload);
            if (carsIndex >= 0) {
